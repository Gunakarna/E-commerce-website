...
const Login = () => {
  ...
  const validatePassword = (password) => {
    const passwordRegix = /[a-zA-Z]/;
    const minLength = 8;
    return password.length >= minLength && passwordRegix.test(password);
  };
  ...
  const onSubmit = async (e) => {
    ...
    if (!emailError && !passwordError) {
      ...
      try {
        ...
        setErrorMessage({ email: 'not valid', password: 'not valid password' });
  ...
  const onGoogleSignIn = (e) => {
    ...
    if (!isSigningIn) {
      ...
      doSignInWithGoogle().catch((err) => {
        setErrorMessage({ email: '', password: err.message });
        setIsSigningIn(false);
      });
    }
  };
